function index_to_shortname( index ){
    return ["xss","xss_tag","sql_injection_differential","path_traversal","os_cmd_injection","unvalidated_redirect"][index];
}

function index_to_severity( index ){
    return {"xss":"high","xss_tag":"high","sql_injection_differential":"high","path_traversal":"high","os_cmd_injection":"high","unvalidated_redirect":"medium"}[index_to_shortname(index)];
}
...
function renderCharts() {
    ...
    c3.generate({
        ...
        regions: [{"class":"severity-high","start":0,"end":4},{"class":"severity-medium","start":5}],
        axis: {
            x: {
                type: 'category',
                categories: ["Cross-Site Scripting (XSS)","Cross-Site Scripting (XSS) in HTML tag","Blind SQL Injection (differential analysis)","Path Traversal","Operating system command injection","Unvalidated redirect"],
                tick: {
                    rotate: 15
                }
            },
