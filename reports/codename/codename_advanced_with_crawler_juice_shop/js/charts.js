function index_to_shortname( index ){
    return ["sql_injection","unvalidated_redirect"][index];
}

function index_to_severity( index ){
    return {"sql_injection":"high","unvalidated_redirect":"medium"}[index_to_shortname(index)];
}
...
function renderCharts() {
    ...
    c3.generate({
        ...
        regions: [{"class":"severity-high","start":0,"end":0},{"class":"severity-medium","start":1}],
        axis: {
            x: {
                type: 'category',
                categories: ["SQL Injection","Unvalidated redirect"],
                tick: {
                    rotate: 15
                }
            },
